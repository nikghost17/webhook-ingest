...
func (s *Service) Ingest(ctx context.Context, evt Event) error {
	...
	// IngestTx atomically runs InsertEvent (ON CONFLICT DO NOTHING) +
	// UpsertCall + IncrementAccountStats in one transaction.
	// If event_id already exists it returns inserted=false and skips all writes,
	// eliminating the TOCTOU race of the previous EventExists + InsertEvent pattern.
	inserted, err := s.store.IngestTx(ctx, rec)
	if err != nil {
		return err
	}
	if !inserted {
	...
	// Only update the in-memory cache after a confirmed DB insert so the cache
	// never reflects a transaction that was rolled back.
